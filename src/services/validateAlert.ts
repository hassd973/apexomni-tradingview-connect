...
import ApexomniConnector from './apexomni/client';
...
export const validateAlert = async (
	...
	if (alertMessage.exchange) {
		const validExchanges = ['apexomni'];
		if (!validExchanges.includes(alertMessage.exchange)) {
			console.error('Exchange name must be apexomni');
			return false;
		}
	}
	...
	// check market if exchange is Apexomni
	if (!alertMessage.exchange || alertMessage.exchange == 'apexomni') {
